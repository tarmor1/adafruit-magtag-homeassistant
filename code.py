...
if use_live:
  try:
    ...
    response          = requests.get(HA_STATES_ENDPOINT_ROOT + "sensor.netatmo_kask_magamistuba_outdoor_temperature",headers=headers)
    outdoor_temp      = round(float(response.json()["state"]),1)

    response          = requests.get(HA_STATES_ENDPOINT_ROOT + "sensor.netatmo_kask_magamistuba_temperature",headers=headers)
    magamistuba_temp  = round(float(response.json()["state"]),1)
  
    response          = requests.get(HA_STATES_ENDPOINT_ROOT + "sensor.netatmo_kask_magamistuba_allkorrus_temperature",headers=headers)
